...
class theme_mb2nl_core_renderer extends core_renderer
{
    ...
    public function theme_part($name, $vars = array())
	{
		...
		{
			$candidate = $CFG->dirroot . '/theme/mb2nl/layout/parts/' . $element;
		}
    ...
    protected function render_custom_menu(custom_menu $menu)
	{
		...
		$content .= '<a class="mobile-home" href="' . new moodle_url($CFG->wwwroot) . '" title="' . theme_mb2nl_theme_setting($PAGE, 'logotitle','New Learning') . '"><i class="' . $startIcon . '"></i></a>';
	   ...
	   $content .= '<ul class="main-menu theme-ddmenu"' . theme_mb2nl_menu_data($PAGE) . '>';
		...
		$content .= '<li class="home-item"><a href="' . new moodle_url($CFG->wwwroot) . '" title="' . theme_mb2nl_theme_setting($PAGE, 'logotitle','New Learning') . '"><i class="' .
		$startIcon . '"></i></a></li>';
