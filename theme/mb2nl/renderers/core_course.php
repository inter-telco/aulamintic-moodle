...
class theme_mb2nl_core_course_renderer extends core_course_renderer {
	...
	protected function theme_mb2nl_coursecat_coursebox_content_image ($url, $course)
	{
		$img = html_writer::empty_tag('img', array('src' => $url, 'class' => ''));
      ...
      if ($isCourseContent)
		{
		   ...
		   if ($course->has_summary())
			...
			$students = get_role_users(theme_mb2nl_theme_setting($PAGE,'studentsroleid',5),$context);
			$students_count = count($students);
			$is_student = theme_mb2nl_is_user_role($course->id,theme_mb2nl_theme_setting($PAGE,'studentsroleid',5),$USER->id);
      ...
      $contentimages = $contentfiles = '';
		...
		if ($isFile)
		{
			...
			foreach ($course->get_course_overviewfiles() as $file)
			{
				...
				if ($isimage)
				{
					$contentimages .= $this->theme_mb2nl_coursecat_coursebox_content_image($url,$course);
				}
				else
				{
			...
			if ($contentimages == '' && $plcImg)
			{
				...
				$contentimages .= $this->theme_mb2nl_coursecat_coursebox_content_image($isPlcImg,$course);
