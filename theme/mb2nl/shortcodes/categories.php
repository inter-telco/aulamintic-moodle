...
function mb2_shortcode_categories($atts, $content= null){
	...
	$categories = theme_mb2nl_shortcodes_categories_get_items($items_opt);
	...
	if ($itemCount>0)
	{
		$output .= theme_mb2nl_shortcodes_content_template($categories, $items_opt);
	}
	else
	{
...
function theme_mb2nl_shortcodes_categories_get_items ($options)
{
	...
	if ($itemCount>0)
	{
		foreach ($categories as $category)
		{
			...
			$imgUrlAtt = theme_mb2nl_shortcodes_categories_get_image($image_options, false, $category->description);
			$imgNameAtt = theme_mb2nl_shortcodes_categories_get_image($image_options, true, $category->description);
			...
			$coursesList = ($category->id && $category->visible) ? coursecat::get($category->id)->get_courses(array('recursive' => false)) : array();
