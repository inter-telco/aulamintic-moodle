 ...
 *
 */
function theme_mb2nl_get_course_activities()
{
			...
			if (is_array($box['links']))
			{
				...
				foreach ($box['links'] as $link)
				{
					...
					if ($allowEdit)
					{
						if (!$link['url'])
						{
							$output .= '<li>' . $icon . '<span>' . $link['title'] . '</span>'. $details . '</li>';
						}
						else
						{
...
function theme_mb2nl_teacher_boxes()
{
	...
	$boxes = array(
		...
		'activities' => array(
			...
			'desc' => '',
			'icon' => 'fa fa-list-ul',
			'links' => theme_mb2nl_links_activities()
		),
		'grade' => array(
...
function theme_mb2nl_contacts_content()
{
	...
	$roleshortname = theme_mb2nl_theme_setting($PAGE,'roleshortname','editingteacher');
		...
		$output .= '<ul>';
			...
			if ($count>0)
			{
				...
				foreach ($teachers as $teacher)
				{
					...
					$picture = $OUTPUT->user_picture($teacher,array('size'=>68));
...
function theme_mb2nl_course_content()
{
	...
	$output = '';
	$banner = theme_mb2nl_course_banner_url();
	...
	$content = theme_mb2nl_wordlimit($COURSE->summary,$limit);


	$output .= '<h3>' . theme_mb2nl_wordlimit($COURSE->fullname,6) . '</h3>';
 ...
 *
 */
function theme_mb2nl_links_activities()
{
	...
	$data = theme_mb2nl_get_course_activities();
