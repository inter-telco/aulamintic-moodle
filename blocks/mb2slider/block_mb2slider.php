...
class block_mb2slider extends block_base
{
	...
	function mb2slider_item($item, $carousel, $counter, $itemCount,  $course = NULL)
	{
		...
		if ($imageUrl)
		{
			...
			// Slide description
			$isText = $course ?  format_text($desc, FORMAT_HTML) : format_text($desc['text'], FORMAT_HTML);
			$showDesc = $course ? ($this->mb2slider_setting('showdesc',7) > 0) : $isText;
			...
			if ($title || $isText)
			{
				...
				$output .= $title ? '<h4 class="mb2slider-title"' . $titleStyle . '>' . $title . '</h4>' : '';
				...
				if ($showDesc)
				{
					...
					$output .= $isText !='' ? '<div class="mb2slider-text" style="font-size:' . $sizecDesc . $sizecDescArrM  . ';">' . $isText . '</div>' : '';
					...
					$output .= ($link !='' && $linkBtn == 1) ? '<a href="' . $link . '" class="mb2slider-btn ' . $linkBtnCls . '" target="' . $linkTarget . '"' . $btnStyle . '>' . $buttonText . '</a>' : '';
					$output .= $navdir == 2 ? '<span class="mb2slider-nextslide"><i class="mb2slider_icon-right-open"></i></span>' : '';
					$output .= $navdir == 2 ? '</div>' : '';
					$output .= (!$carousel && $link !='' && $linkBtn == 1) ? '<a href="' . $link . '" class="mb2slider-btn-mobile" target="' . $linkTarget . '">&#43;</a>' : '';
